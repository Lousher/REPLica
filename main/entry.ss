  ...
  (lambda (file)
    (syntax-case file ()
	 ...
	 #`(with-fullscreen
		...
		#,(with-syntax ([(scenes ...) (datum->syntax #'with-fullscreen scripts)])
		    #`(let* ([scenes:all (list scenes ...)]
			     [scene:next (lambda ()
					   (unless (atom? scenes:all)
					     (set! scenes:all (cdr scenes:all))))]
			     [scene:current (let ([cur (car scenes:all)])
					      (lambda ()
						(unless (atom? scenes:all)
						  (set! cur (car scenes:all)))
						cur))]
			     [scene:render ((scene:current))])
			(drawing-loop
			 [(when (and (IsMouseButtonPressed MOUSE_BUTTON_LEFT) (not (atom? scenes:all)))
			    (scene:next)
			    (set! scene:render ((scene:current)))
			    )] ;updating
			 [(scene:render)] ;drawing
			 [(clear:location)
			  (clear:characters)
			  (clear:text)
			  (clear:voice)] ;cleaning
			 )
			))))))])))
