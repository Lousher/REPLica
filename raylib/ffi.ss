(library (raylib ffi)
  ...
  (define-ffi ClearWindowState (unsigned-int) void)
  ...
  (define-ffi SetWindowPosition (int int) void)
  ...
  (define-ffi LoadImageFromMemory (string u8* int) (& Image))
  (define-ffi ImageFlipVertical ((* Image)) void)
  (define-ffi ImageFlipHorizontal ((* Image)) void)
  (define-ffi LoadImageColors ((& Image)) (* Color))
  (define-ffi ImageColorGrayscale ((* Image)) void)
  (define-ffi UnloadImageColors ((* Color)) void)
  ...
  (define-ffi IsImageValid ((& Image)) boolean)
  ...
  ;; Sound
  (define-ffi LoadWaveFromMemory (string u8* int) (& Wave))
  (define-ffi LoadSoundFromWave ((& Wave)) (& Sound))
  (define-ffi UnloadWave ((& Wave)) void)
  ...
  (define-ffi LoadMusicStreamFromMemory (string u8* int) (& Music))
  (define-ffi UnloadMusicStream ((& Music)) void)
  (define-ffi PlayMusicStream ((& Music)) void)
  (define-ffi PauseMusicStream ((& Music)) void)
  (define-ffi ResumeMusicStream ((& Music)) void)
  ...
  (define-ffi DrawTextPro ((& Font) string (& Vector2) (& Vector2) float float float (& Color)) void)
  ...
  (define-ffi LoadFontData (u8* int int u32* int int) (* GlyphInfo))
  (define-ffi UnloadFontData ((* GlyphInfo) int) void)
  (define-ffi GenImageFontAtlas ((* GlyphInfo) (* *Rectangle) int int int int) (& Image))
  ...
  (define-ffi GenImageColor (int int (& Color)) (& Image))

  (define-ffi ImageAlphaMask ((* Image) (& Image)) void)
  (define-ffi ImageFormat ((* Image) int) void)

  (define-ffi GetWindowScaleDPI () (& Vector2))
  (define-ffi GetRenderWidth () int)
  (define-ffi GetRenderHeight () int)
  (define-ffi DrawRectangleLines (int int int int (& Color)) void)
  (define-ffi GetMouseX () int)
  (define-ffi GetMouseY () int)
  (define-ffi GetMousePosition () (& Vector2))

  (define-ffi GetScreenToWorld2D ((& Vector2) (& Camera2D)) (& Vector2))

  ;; detection
  (define-ffi CheckCollisionPointRec ((& Vector2) (& Rectangle)) boolean)

  (define-ffi GetMouseWheelMove () float)
  (define-ffi GetMouseWheelMoveV () (& Vector2))
  ...
  (define RenderTexture-width
    (lambda (fptr)
      (ftype-ref RenderTexture (texture width) fptr)))

  (define RenderTexture-height
    (lambda (fptr)
      (ftype-ref RenderTexture (texture height) fptr)))
  ...
  (define make-vector2
    (lambda (x y)
      (make-Vector2 (inexact x) (inexact y))))
  )
